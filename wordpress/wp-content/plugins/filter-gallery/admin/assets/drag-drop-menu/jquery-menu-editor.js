...
(function ($){
	 ...
	 */
	jQuery.fn.sortableLists = function( options )
	{
		// Local variables. This scope is available for all the functions in this closure.
		var jQBody = jQuery( 'body' ).css( 'position', 'relative' ),
			...
			setting = jQuery.extend( true, {}, defaults, options ),

			// base element from which is counted position of draged element
			base = jQuery( '<' + setting.listSelector + ' />' )
				.prependTo( jQBody )
			...
			hintWrapper = jQuery( '<' + setting.listSelector + ' />' )
		...
		function startDrag( e, el, rEl )
		{
			...
			var placeholderNode = state.placeholderNode = jQuery( '#sortableListsPlaceholder' );  // jQuery object && document node
			...
			if ( state.isDragged ) {
				...
				if ( doc.scrollTop() > state.rootEl.offset.top - 10 && e.clientY < 50 ) {
					if ( ! state.upScroll ) { // Has to be here after cond. e.clientY < 50 cause else unsets the interval
						...
						jQuery( 'html, body' ).each(
							function( i )
							{
										jQuery( this ).scrollTop( jQuery( this ).scrollTop() - setting.scroll );
							}
						);
						setCursorPos( e );
					}
		...
		function endDrag( e )
		{
			...
			cEl.el.animate(
				{ left: offset.left - state.cEl.mL, top: offset.top - state.cEl.mT },
				250,
		...
		function elFromPoint( x, y )
		{
			...
			if ( isRelEFP === null ) {
				...
				if ( (s = state.doc.scrollTop()) > 0 ) {
					isRelEFP = ( (res = document.elementFromPoint( 0, s + jQuery( window ).height() - 1 ) ) == null
					|| res.tagName.toUpperCase() == 'HTML');  // IE8 returns html
				}
				if ( (s = state.doc.scrollLeft()) > 0 ) {
					isRelEFP = ( (res = document.elementFromPoint( s + jQuery( window ).width() - 1, 0 ) ) == null
					|| res.tagName.toUpperCase() == 'HTML');  // IE8 returns html
				}
			}
		...
		{
			if ( jQuery( '#sortableListsHintWrapper', state.rootEl.el ).length ) {
		...
		function tidyEmptyLists()
		{
			...
			jQuery( setting.listSelector, state.rootEl.el ).each(
				function( i )
				{
					if ( ! jQuery( this ).children().length ) {
						  jQuery( this ).prev( 'div' ).children( '.sortableListsOpener' ).first().remove();
						  jQuery( this ).remove();
					}
				}
			);
	 ...
	 * @param setting
	 */
	jQuery.fn.iconOpen = function(setting){
	 ...
	 * @param setting
	 */
	jQuery.fn.iconClose = function(setting) {
	 ...
	 * @return {array} Array
	 */
	jQuery.fn.sortableListsToJson = function (){
		var arr = [];
	 ...
	 * @param {int} depth
	 */
	jQuery.fn.updateButtons = function (depth){
		...
		if (level === 0) {
			...
			removelast.push( 'Out' );
			jQuery( this ).children( 'li' ).hideButtons( ['Out'] );
		}
		jQuery( this ).children( 'li' ).each(
			function () {
				var $li = jQuery( this );
		...
		jQuery( this ).children( 'li:first' ).hideButtons( removefirst );
		jQuery( this ).children( 'li:last' ).hideButtons( removelast );
	};
	/**
	 * @description Hide the buttons at the item <li>
	 * @param {Array} buttons
	 */
	jQuery.fn.hideButtons = function(buttons){
		for (var i = 0; i < buttons.length; i++) {
			jQuery( this ).find( '.btn-group:first' ).children( ".btn" + buttons[i] ).hide();
		}
	};
}(jQuery));
/**
...
function MenuEditor(idSelector, options) {
	...
	// iconpicker plugin
	var iconPicker = jQuery( '#' + idSelector + '_icon' ).iconpicker( iconPickerOpt );
	...
	jQuery( document ).on(
		...
		function (e) {
			e.preventDefault();
			itemEditing = jQuery( this ).closest( 'li' );
		...
		function (e) {
			...
			if ($prev.length > 0) {
				...
				if ($ul.length > 0) {
					...
					var $ul = jQuery( '<ul>' ).addClass( 'pl-0' ).css( 'padding-top', '10px' );
					$prev.append( $ul );
					$ul.append( $li );
	...
	function TButton(attr) {
		return jQuery( "<a>" ).addClass( attr.classCss ).addClass( 'clickable' ).attr( "href", "#" ).html( attr.text );
	}

	function TButtonGroup() {
		var $divbtn  = jQuery( '<div>' ).addClass( 'btn-group float-right mr-2' );
	...
	function createMenu(arrayItem, depth) {
		var level = (typeof (depth) === 'undefined') ? 0 : depth;
		var $elem = (level === 0) ? $main : jQuery( '<ul>' ).addClass( 'pl-0' ).css( 'padding-top', '10px' );
		jQuery.each(
			arrayItem,
			function (k, v) {
				var isParent   = (typeof (v.children) !== "undefined") && (jQuery.isArray( v.children ));
				var itemObject = {text: "", href: "", icon: "empty", target: "_self", title: ""};
				var temp       = jQuery.extend( {}, v );
				if (isParent) {
					delete temp['children'];
				}
				jQuery.extend( itemObject, temp );
				var $li = jQuery( '<li>' ).addClass( 'list-group-item pr-0 m-1' );
				$li.data( itemObject );
				var $div    = jQuery( '<div>' ).css( 'overflow', 'auto' );
				var $i      = jQuery( '<i>' ).addClass( v.icon );
				var $span   = jQuery( '<span>' ).addClass( 'txt' ).append( v.text ).css( 'margin-right', '5px' );
	...
	function TOpener(li){
		var opener = jQuery( '<span>' ).addClass( 'sortableListsOpener ' + options.opener.openerClass ).css( options.opener.openerCss )
				.on(
					'mousedown touchstart',
					function (e){
						var li = jQuery( this ).closest( 'li' );
	...
	this.update = function(){
		...
		$form.find( '.item-menu' ).each(
			function(){
				$cEl.data( jQuery( this ).attr( 'name' ), jQuery( this ).val() );
			}
		);
	...
	this.add = function(){
		...
		var textItem = jQuery( '<span>' ).addClass( 'txt' ).text( data.text );
		var iconItem = jQuery( '<i>' ).addClass( data.icon );
		var div      = jQuery( '<div>' ).css( {"overflow": "auto"} ).append( iconItem ).append( "&nbsp;" ).append( textItem ).append( btnGroup );
		var $li      = jQuery( "<li>" ).data( data );
