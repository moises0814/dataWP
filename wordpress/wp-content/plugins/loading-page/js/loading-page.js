...
(function(){
	...
	{
		var style_tag = jQuery('[id="loading-page-inline-style"]');
		if(style_tag.length) style_tag.remove();
		else jQuery('html').append('<style>body{'+((lp.options['modifyDisplayRule']*1) ? 'display:block' : 'visibility:visible')+';}</style>');
	};
    ...
    lp['displayScreen'] = function()
    {
		...
		if(lp.options['closeBtn'])
		{
			...
			close_btn.on('click',function(){lp_close_btn = true; if(!destroyed) lp.destroyLoader(); else lp.onLoadComplete();});
