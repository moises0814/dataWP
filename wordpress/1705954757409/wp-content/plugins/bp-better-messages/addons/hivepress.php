...
if ( ! class_exists( 'Better_Messages_HivePress' ) ) {
    ...
    class Better_Messages_HivePress
    {
        ...
        public function render_small_listing_button(){
            ...
            $btn_label = esc_html_x('Send Message', 'HivePress Integration (Private Message)', 'bp-better-messages');
            ...
            if( ! is_user_logged_in() && ! Better_Messages()->guests->guest_access_enabled() ){
                ...
                $subject = sprintf(esc_html_x('Question about listing "%s"', 'HivePress Integration (Listing page)', 'bp-better-messages'), get_the_title());
                ...
                if( $post_type === 'hp_booking' ){
                    ...
                    if( $listing ) {
                        $subject = sprintf(esc_html_x('Question about booking "%s" of "%s"', 'HivePress Integration (Booking item)', 'bp-better-messages'), get_the_title($booking_id), get_the_title( $listing ));
            ...
            if( $hp_order ){
                ...
                if( $order ){
                    if ( $order->get_status() === 'processing' ) :
                        ...
                        $subject = sprintf(esc_attr_x('Question about order #%d', 'HivePress Integration (Order Page)', 'bp-better-messages'), $order_id);

                        if ( get_current_user_id() === $buyer_id ) {
                            $btn_label = esc_html(hivepress()->translator->get_string('contact_seller'));
                            $user_id = $hp_order->get_seller__id();
                        } else {
                            $btn_label = esc_html(hivepress()->translator->get_string('contact_buyer'));
                            $user_id = $buyer_id;
                        }
        ...
        public function render_big_listing_button(){
            $btn_label = esc_html_x('Send Message', 'HivePress Integration (Reply To Listing)', 'bp-better-messages');
            ...
            if( ! is_user_logged_in() && ! Better_Messages()->guests->guest_access_enabled() ){
                ...
                $current_user_id = Better_Messages()->functions->get_current_user_id();
                $user_id = (int) get_the_author_meta('ID');
                $subject = sprintf(esc_html_x('Question about your listing "%s"', 'HivePress Integration (Product page)', 'bp-better-messages'), get_the_title());
